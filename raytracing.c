...
int main(void) {
    ...
    Ray rays[SHADOW_CIRCLE_NUMBER][RAYS_NUMBER];

    int running = 1;
    int drag = 0;
    SDL_Event e;

    int shadow_circle_index = -1;
    int circle_index = -1;
	...
	while (SDL_PollEvent(&e)) {
	    ...
	    int prev_shadow_circle_index;
	    int prev_circle_index;
	    // check the right circle pressed
		...
		for(int i = 0 ; i < SHADOW_CIRCLE_NUMBER ; i++) {
			...
			prev_shadow_circle_index = shadow_circle_index;
			circle_index = -1;
			break;
		    }
		}
		for(int i = 0 ; i < CIRCLE_NUMBER ; i++) {
			...
			circle_index = i;
			prev_circle_index = circle_index;
	    ...
	    if (shadow_circle_index != -1) {
	        shadow_circles[shadow_circle_index].color = COLOR_RED;
	    }
	    else {
		shadow_circles[prev_shadow_circle_index].color = COLOR_WHITE;
	    }
	    if (circle_index != -1) {
	        circles[circle_index].color = COLOR_RED;
	    }
	    else {
		circles[prev_circle_index].color = COLOR_WHITE;
	    }
	...
	for (int i = 0 ; i < SHADOW_CIRCLE_NUMBER ; i++) 
	    generate_rays(&shadow_circles[i], rays[i]);

	for (int i = 0 ; i < CIRCLE_NUMBER ; i++) 
	    FillCircle(surface,&circles[i]);

	for(int j = 0 ; j < SHADOW_CIRCLE_NUMBER ; j++)
	    for (int i = 0 ; i < CIRCLE_NUMBER ; i++) 
		check_collision(&circles[i], rays[j], RAYS_NUMBER);

	for(int i = 0 ; i < SHADOW_CIRCLE_NUMBER ; i++)
	    FillRays(surface, rays[i]);
